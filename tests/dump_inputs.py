...
#:
#: ⚠ 对**阈值型**候选尤其重要：8.10 那两个显影的构型（`RICH`/`POOR`）读的是
#:   `gold_remaining > 阈值`。它们在**迷雾日志的低估值下仍显影 534/1500**，
#:   所以那个数字是「低估值下的下界」，**不是本门的灵敏度上限**。
#:   ⇒ 以后测任何阈值型快照候选，**必须用全信息日志**，否则阈值可能永不触发、
#:   门会以「无分歧」的形式给出假绿灯 —— 这正是本门原本失明的那种形状。
