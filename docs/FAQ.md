...
![Image](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=M2QxZTU2YTY3NjU4OGZhYzUxZTA3YTE2YzQyNzdiYTZfMDQ3MmI1Y2Q2N2Q1ZjNhMGI3YzJkZjNlZGRkMWQxNjFfSUQ6NzY2MDA5MzQxNzA2MTUwMjE0Ml8xNzg2MDY5NzY4OjE3ODYxNTYxNjhfVjM)
...
![Image](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=Njg3YmIxZDEyNWZhM2Y4MTcxYzc4OTExYjkwNjM0NmFfNzVlYmUyYmQ3OTBkN2RjM2RlNDBhZjk3ODk2ODg5MzRfSUQ6NzY2MzA2NTgwMjAwNjA1NjIwM18xNzg2MDY5NzY4OjE3ODYxNTYxNjhfVjM)

A：公测和初赛会固定用这个区域划分方法～

![Image](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=OTg0ZDg0OWRkNTI1NGQwYmQ0MmFkZjI0MWI1ZGU0ZDJfYWVlZTJmZDhkNjc5MjhjOGQ4ODA2MmMyZmI0YzliMmZfSUQ6NzY2MzA2ODIwMzEwNDM1NzM1N18xNzg2MDY5NzY4OjE3ODYxNTYxNjhfVjM)

> **【本地实测补充 · 非官方原文】区域划分已解码并验证**（2026-08-09；验证数据 = 开发机 `/share/data.tar.gz` 内 3 份全信息日志 `data/full/g0-g2.txt`）
>
> 上图的 5 个区域是**绕中央 9×9 的风车形铺砌**（90° 旋转对称），在 17×17 上无重叠、无空隙：
>
> | 区域 | 范围（row / col，闭区间，0-based） | 尺寸 | 格数 |
> |---|---|---|---|
> | 1 中央 | row 4-12, col 4-12 | 9×9 | 81 |
> | 2 上 | row 0-3, col 0-12 | 4×13 | 52 |
> | 3 左 | row 4-16, col 0-3 | 13×4 | 52 |
> | 4 下 | row 13-16, col 4-16 | 4×13 | 52 |
> | 5 右 | row 0-12, col 13-16 | 13×4 | 52 |
>
> 合计 81 + 52×4 = 289 = 17²。**关键：四条边臂是风车咬合，不是十字对称**——
> 上臂让出右 4 列给区域 5，右臂让出下 4 行给区域 4，下臂让出左 4 列给区域 3，左臂让出上 4 行给区域 2。
> 按十字/象限直觉实现会在四个角块上系统性错区。
>
> **采样时点（实测）**：`gold_remaining` 与 `occupants` 取的是**本轮 start 时刻**——刷金刷弹之后、
> 任何移动之前，即与玩家当轮拿到的棋盘一致；不是窗口结尾 `end[r-1]`（该假设实测仅 14/99 命中）。
> `occupants` = 4 个玩家角色 + 7 个 NPC 按所在区域计数（含 NPC，与官方答复一致）。
>
> **验证强度**：3 份全信息日志 × 99 个快照 = 297 个样本，`gold_remaining` 与 `occupants` 全部逐区精确吻合，
> 且区域编号为恒等映射（无需置换）。复现脚本见 `sim/`。
...
![Image](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=ZGZmOTcyNjQ2ZjAxNzExNDIxOTI5YjQyZWZhZmZmOTdfYzFmMzY1NjcyYmM5YWYwYTQ3NjQ4ODdjMjAxNjJjNmZfSUQ6NzY2NTE5NjYwNTE1MDMwMTEzMl8xNzg2MDY5NzY4OjE3ODYxNTYxNjhfVjM)
