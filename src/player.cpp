...
// ── 以下三个开关此前**没有 #ifndef 默认块**, 靠「未定义标识符在 #if 里求值为 0」隐式为 0。
//    补成显式默认, 与上面所有开关同约定 ⇒ 默认值从此写在源码里, 不依赖预处理器的隐式行为。
#ifndef PV_SKIPSEEN
#define PV_SKIPSEEN 1    // 1=冷层扫描逐行跳过「该行全部格已观测」的行。**按构造无损**: 只在该行
                         // 没有任何未观测格时才跳 ⇒ 不可能漏掉信息。判据用现成的 g_s.seen[](每行
                         // 一个 uint32_t), 19 条指令替掉一次 5x5 扫描的 175-200 条。
                         // 验收: 232 局逐字节恒等(旧族 0/120 + 决赛族 0/112, 后者 14 张锚点压墙
                         // ⇒ 真正激活了唯一消费者 fixAnchor, 不是平凡通过);
                         // 臂 T(删掉 slowTick)232/232 全被抓, 金币最坏 -955/-1144 ⇒ 判据有判别力。
                         // 效应量: 旧(不对称)族 10 图 P90 均值 100.3→75.0、种子间 sd 20.4→7.7。
                         // ⚠ 该效应量在**对称族**上未复测 ⇒ 「-25.3ns」只在旧族口径内成立。
                         // ⚠ 官方三图上金币逐字节恒等(0/36)⇒ 对胜率零影响; 但 .text 4113→4241B
                         // 的布局代价实测约 +3~4ns P90(同代码多个 PV_PAD 值分组比较, 两组零重叠)。
#endif
#ifndef PV_NOLOCK
#define PV_NOLOCK 1      // 1=**不背图**: 指纹淘汰照跑, 但唯一候选时按陌生图处置(map_id=-2),
                         // 不再执行「锁图直灌」(:485 的 bpw = 0xFFFC0001u | BAKED_W[m][r])。
                         // ⇒ BAKED_W 的内容从此不进入任何运行期状态; map_id 永不 >=0
                         // ⇒ map1 烘焙开局路线(:510 的 map_id==0 分支)自动成为死代码。
                         // **所有者决定**: 三张官方图一律当新图打, 理由是初赛全为新图,
                         // 而背图会让「竞技场量到的数」描述一条初赛不会走的路径。
                         // ⚠ 已实测代价(真实对局, 前 16 名 14 支 x 3 图 x 2 批 = 84 局, 同对手同图配对):
                         //   认地图开 39/42=92.9% vs 关 37/42=88.1%; 我方净分优势平均 **-100 金/局**;
                         //   两局翻转(T-1 map2、Zzz map2)。⇒ 这 100 金是初赛必付的, 因为新图认不出。
                         // ⚠ 与之矛盾的旧读数: sim 上「完美地图知识值 -1.18±8.32 金(n=324)」——
                         //   那是对**弱陪练**测的, 座位与小优势在那种局里不起作用 ⇒ regime 不同。
                         //   **以真实对局的 -100 金为准。**
                         // ⭐ 一条尚未实测的机制线索(:489): map_id<0 时 round 0 会 vp_buy=2(买 9x9 视野),
                         //   而 T-1 实测 12/12 局视野费全 0 ⇒ 陌生图上我方每局多付视野费, 可能是那 100 金的一块。
                         // 0 = 恢复背图(旧行为), 用于 A/B 与回滚。
#endif
#ifndef PV_NOROUTE
#define PV_NOROUTE 0     // 1=另外关掉 map1 专用烘焙开局路线(:510)。**默认 0 且在 PV_NOLOCK=1 下无意义**
                         // (map_id 永不为 0 ⇒ 那条分支已是死代码), 保留它只为可复现那次分离测量:
                         // 官方三图合池 -1.69±11.49 金、map2/map3 精确 0/12 局变化(门控 map_id==0 自证)
                         // ⇒ 去锁那 -54.47 金里路线只占 3%, 主体 97% 是墙表。
                         // ⚠ 三项均在 2σ 内 ⇒ 该拆分是**方向**而非定量结论。
                         // ⚠ 并注意档案里 -0.500±0.126 那个"精确"读数是在**角落克隆代理语料**上量的,
                         // 在它真正会发生的地方(map1)实测 -5.08±**34.44** ⇒ 精度差 273 倍。
#endif
...
#ifndef PV_PAD
// ⚠ PV_PAD 的正确值**依赖开关组合**, 因为各组合的 .text 体积不同:
//   PV_SKIPSEEN=0 PV_NOLOCK=0 (旧落库构型, .text 4113B) ⇒ **192** ⇒ 入口 0x1950
//   PV_SKIPSEEN=1 PV_NOLOCK=0 (.text 4241B)             ⇒ **200** ⇒ 入口 0x19d0
//   PV_SKIPSEEN=1 PV_NOLOCK=1 (**当前默认**, .text 4177B) ⇒ **228** ⇒ 入口 0x1990
// ⇒ ⛔ 对齐的目标是「入口 mod 64 == 0x10」这个**性质**, 不是「PV_PAD 取同一个数」这个**参数**。
//    做 A/B 时给两臂设同一个 PV_PAD 反而会把它们落在不同的 mod64 档上。
#define PV_PAD 228       // 默认值对应 PV_SKIPSEEN=1 + PV_NOLOCK=1 + PV_ACTOR=1 + PV_RTAB=1
#endif
#define PV_STR2(x) #x
